...
def ego_network_clustering(neighbors_fpath, clusters_fpath, max_related=300, num_cores=32):
    ...
    with codecs.open(clusters_fpath, "w", "utf-8") as output, Pool(num_cores) as pool:
        ...
        for i, ego_network in enumerate(pool.imap_unordered(get_ego_network, G.index)):
            ...
            sense_num = 1
            for label, cluster in sorted(aggregate_clusters(ego_network).items(), key=lambda e: len(e[1]),
                                         reverse=True):
                output.write("{}\t{}\t{}\t\n".format(
                    ego_network.name,
                    sense_num,
                    ", ".join(
                        ["{}:{:.4f}".format(n, w) for w, n in
                         sorted([(ego_network.node[c_node]["weight"] / WEIGHT_COEF, c_node) for c_node in cluster],
                                reverse=True)]
                    )))
                sense_num += 1
